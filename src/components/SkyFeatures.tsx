...
export function TonightEvent({
  ...
  return (
    ...
    >
      <div className="rounded-2xl border border-white/12 bg-[#1b1038]/60 p-3 backdrop-blur-md">
...
export function AuroraBand() {
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(to top, rgba(251,146,60,0.15) 0%, rgba(236,110,120,0.09) 30%, rgba(150,100,220,0.05) 60%, transparent 100%)",
      }}
    />
  );
}
...
export function WishComet({
  ...
  return (
    ...
    >
      ...
      />
      <span className="pointer-events-none absolute left-1/2 top-full mt-1 -translate-x-1/2 whitespace-nowrap rounded-full border border-white/10 bg-[#1b1038]/70 px-2 py-0.5 text-[10px] text-foreground/90 opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100">
    ...
    <AnimatePresence>
      {open && starter && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="radius-sheet absolute inset-0 z-40 flex items-center justify-center bg-[#171033]/60 backdrop-blur-sm"
          onClick={onClose}
        >
          <motion.div
            ...
            className="mx-4 max-w-xs rounded-2xl border border-amber-300/25 bg-[#211448]/95 p-5 text-center shadow-2xl"
